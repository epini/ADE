...
addpath('C:\Users\ernes\Desktop\PostDoc\MC\Anisotropy\Giusfredi formulas test')

%% Load MAT file
matFile = 'tr_transmission_curves_paper09.mat';   % set full path if needed
S = load(matFile);
...
L      = 10*1e3;         % still not in MAT -> keep here or add to meta in Python
...
plot(t_ps, smooth((T_ADE  - MC_pencil) ./ (MC_pencil + eps0), 10), 'LineWidth', 1.2);
% plot(t_ps, (T_CORR - MC_pencil) ./ (MC_pencil + eps0), 'LineWidth', 1.2);
plot(t_ps, smooth((T_ALER - MC_pencil) ./ (MC_pencil + eps0), 10), 'LineWidth', 1.2);
xlabel('$t$ [ps]','Interpreter','latex','FontSize',14);
...
legend({'ADE $\lambda$','ADE Alerstam'}, ...
       'Interpreter','latex','Location','best','FontSize',12);
xlim([500 5000]); ylim([-0.25 0.25]);
...
%% Fit esponenziale tra t=3000 e t=4500

% intervallo di fit
fit_range = (t_ps >= 3000 & t_ps <= 4500);

t_fit = t_ps(fit_range);

% --- Fit per la curva teorica ---
y_fit_theory = T_ADE(fit_range);
fit_theory = fit(t_fit,y_fit_theory,'exp1');

y_fit_theory2 = T_ALER(fit_range);
fit_theory2 = fit(t_fit,y_fit_theory2,'exp1');

% --- Fit per la curva MC ---
y_fit_MC = MC_pencil(fit_range);
fit_MC = fit(t_fit, y_fit_MC,'exp1');

y_fit_MC2 = MC_iso(fit_range);
fit_MC2 = fit(t_fit, y_fit_MC2,'exp1');

% Tau dai due fit
tau_theory = -1/fit_theory.b;
tau_theory2 = -1/fit_theory2.b;
tau_MC = -1/fit_MC.b;
tau_MC2 = -1/fit_MC2.b;

disp(['Tau teoria ADE = ', num2str(tau_theory), ' ps'])
disp(['Tau teoria Alerstam = ', num2str(tau_theory2), ' ps'])
disp(['Tau MC     = ', num2str(tau_MC), ' ps'])
disp(['Tau MC iso = ', num2str(tau_MC2), ' ps'])